...
namespace al {
	using LegacyEventHandler = std::function<void(const ALLEGRO_EVENT&)>;
	...
	public:
		...
		~LegacyEventDispatcher() = default;
		
		void setDefaultHandler(LegacyEventHandler handler) {
			defaultHandler = std::move(handler);
		}
		void setEventTypeHandler(ALLEGRO_EVENT_TYPE evType, LegacyEventHandler handler) {
			eventTypeHandler[evType] = std::move(handler);
		}
		void setEventValueHandler(EventDiscretizerId discrId, int64_t value, LegacyEventHandler handler) {
	...
	private:
		...
		LegacyEventHandler defaultHandler;
		std::unordered_map<ALLEGRO_EVENT_TYPE, LegacyEventHandler> eventTypeHandler;
		...
		std::unordered_map<EventDiscretizerId, std::unordered_map<int64_t, LegacyEventHandler>> eventValueHandler;
	};
}

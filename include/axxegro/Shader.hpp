...
namespace al {
    ...
    public:
        ...
        {
            if constexpr(std::is_same_v<typename Vec::ValueType, int>) {
                return al_set_shader_int_vector(name.c_str(), Vec::NumElements, (typename Vec::ValueType*)vec.data(), vec.size());
            } else if constexpr(std::is_same_v<typename Vec::ValueType, float>) {
                return al_set_shader_float_vector(name.c_str(), Vec::NumElements, (typename Vec::ValueType*)vec.data(), vec.size());

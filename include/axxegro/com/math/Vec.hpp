...
/*
 * for inlining the crazy metaprogramming stuff in debug builds
 * so that performance doesn't go to hell completely
 */
#ifdef _MSC_VER
...
namespace std {
	...
	template<::al::vec::InstOfBaseCoords T>
	struct tuple_size<T> : std::integral_constant<std::size_t, T::NumDimensions> {};
	
	template<std::size_t I, ::al::vec::InstOfBaseCoords T>
	struct tuple_element<I, T> {
		using type = typename T::ElementType;
	};
	
	template<std::size_t I, ::al::vec::InstOfBaseArrCoords T>
	...
	template<std::size_t I, ::al::vec::InstOfBaseNamedCoords T>
	AXXEGRO_FORCE_INLINE inline constexpr auto get(T& v) -> std::tuple_element_t<I, T>& {
		static_assert(I>=0 && I<T::NumDimensions, "std::get<al::Vec> subscript out of range");
		return al::vec::GetNamedCoordAt<T, I>::from(v);
	}
	...
	namespace vec {
			...
			public std::conditional_t<N==3, AddCrossProduct<BaseVec<T, N, TImpl>>, Empty>
		{
			...
			AXXEGRO_FORCE_INLINE constexpr void forEach(Func fn) {
				[this, fn]<std::size_t... Idxs>([[maybe_unused]] std::index_sequence<Idxs...> seq) {
					(fn(this->getElem<Idxs>()), ...);
			...
			AXXEGRO_FORCE_INLINE constexpr void pairwiseOp(const BaseVec& other, Func fn) {
				...
				[this, other, fn]<std::size_t... Idxs>([[maybe_unused]] std::index_sequence<Idxs...> seq) {
					(fn(this->getElem<Idxs>(), other.getElem<Idxs>()), ...);
				}(std::make_index_sequence<NumElements>{});
			}
			
			template<std::invocable<ValueType&, const ValueType&> Op>
			AXXEGRO_FORCE_INLINE [[nodiscard]] constexpr ValueType foldl(ValueType initial, Op op) const {
				forEach([&](const ValueType& v){op(initial, v);});
				return initial;
			}
			
			template<std::invocable<ValueType&, const ValueType&> Op>
			AXXEGRO_FORCE_INLINE [[nodiscard]] constexpr ValueType foldr(ValueType initial, Op op) const {
			...
			[[nodiscard]] constexpr BaseVec rotated() const {
				...
				[&]<std::size_t... Idxs>([[maybe_unused]] std::index_sequence<Idxs...>){
					((ret.getElem<(RVal + Idxs) % N>() = getElem<Idxs>()), ...);
				}(std::make_index_sequence<NumElements>{});
			...
			[[nodiscard]] constexpr BaseVec<TValue2, N, TImpl2> as() const {
				...
				[this, &ret]<std::size_t... Idxs>([[maybe_unused]] std::index_sequence<Idxs...> seq){
					((ret.template getElem<Idxs>() = this->getElem<Idxs>()), ...);
				}(std::make_index_sequence<NumElements>{});

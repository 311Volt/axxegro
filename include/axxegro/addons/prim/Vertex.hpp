...
namespace al {
	...
	constexpr int MaxCustomVertexElements = ALLEGRO_PRIM_ATTR_NUM;
	...
	namespace detail {
		...
		template<>
		struct VtxElemStorageType<float> {
			constexpr ALLEGRO_PRIM_STORAGE operator()()
			{ return ALLEGRO_PRIM_FLOAT_1; }
		};

		template<>
		struct VtxElemStorageType<Vec2f> {
			constexpr ALLEGRO_PRIM_STORAGE operator()()
			{ return ALLEGRO_PRIM_FLOAT_2; }
		};

		template<>
		struct VtxElemStorageType<Vec3f> {
			constexpr ALLEGRO_PRIM_STORAGE operator()()
			{ return ALLEGRO_PRIM_FLOAT_3; }
		};

		template<>
		struct VtxElemStorageType<Vec4f> {
			constexpr ALLEGRO_PRIM_STORAGE operator()()
			{ return ALLEGRO_PRIM_FLOAT_4; }
		};

		template<>
		struct VtxElemStorageType<Vec2<short>> {
			constexpr ALLEGRO_PRIM_STORAGE operator()()
			{ return ALLEGRO_PRIM_SHORT_2; }
		};

		template<>
		struct VtxElemStorageType<Vec4<short>> {
			constexpr ALLEGRO_PRIM_STORAGE operator()()
			{ return ALLEGRO_PRIM_SHORT_4; }
		};

		template<>
		struct VtxElemStorageType<Vec4b> {
			constexpr ALLEGRO_PRIM_STORAGE operator()()
			{ return ALLEGRO_PRIM_UBYTE_4; }
		};

		template<>
		struct VtxElemStorageType<NormalizedVec<Vec2<short>>> {
			constexpr ALLEGRO_PRIM_STORAGE operator()()
			{ return ALLEGRO_PRIM_NORMALIZED_SHORT_2; }
		};

		template<>
		struct VtxElemStorageType<NormalizedVec<Vec4<short>>> {
			constexpr ALLEGRO_PRIM_STORAGE operator()()
			{ return ALLEGRO_PRIM_NORMALIZED_SHORT_4; }
		};

		template<>
		struct VtxElemStorageType<NormalizedVec<Vec2<unsigned short>>> {
			constexpr ALLEGRO_PRIM_STORAGE operator()()
			{ return ALLEGRO_PRIM_NORMALIZED_USHORT_2; }
		};

		template<>
		struct VtxElemStorageType<NormalizedVec<Vec4<unsigned short>>> {
			constexpr ALLEGRO_PRIM_STORAGE operator()()
			{ return ALLEGRO_PRIM_NORMALIZED_USHORT_4; }
		};

		template<>
		struct VtxElemStorageType<NormalizedVec<Vec4b>> {
			constexpr ALLEGRO_PRIM_STORAGE operator()()
			{ return ALLEGRO_PRIM_NORMALIZED_UBYTE_4; }
		};
...
#undef AXX_CREATE_SIMPLE_VTX_ELEM
	...
	template<>
	struct VertexAttrArrGetter<BasicVertex> {
